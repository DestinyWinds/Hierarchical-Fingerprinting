...
parser.add_argument('--source_path', default="../../npz_dataset/Closed_2tab", help='Undefended dataset')
parser.add_argument('--output_path', default="../results/regulator_Closed_2tab", help='Output path')
...
def regulator_download(target_trace, target_trace_l):
    ...
    for i in range(added_packets):
        pkt_time = i * .1
        output_trace.append((pkt_time, -1))
    ...
    while True:
        ...
        if (queue_length == 0 and padding_packets >= padding_budget):
            ...
            output_trace.append((current_time, -1))
            padding_packets += 1
        else:
            # real packet to send
...
def regulator_upload_full(download_trace, upload_trace, upload_trace_l):
    ...
    length_sequence = [1 for _ in range(len(output_trace))]  # [512] * len(output_trace)
...
if __name__ == '__main__':
    ...
    if (SAVE_PICKLE):
        ...
        data = np.load(args.source_path + "/data.npz")
        X = data["X"]
        y = data["y"]
        ...
        for p, label in tqdm.tqdm(zip(X, y)):
            ...
            for i in range(len(p)):
                ...
                trace_line = (timestamp, direction, label, "")
                if timestamp < CUTOFF_TIME:
                    trace.append(trace_line)

            if len(trace) > CUTOFF_LENGTH:
                trace = trace[:CUTOFF_LENGTH]

            simulated = simulate(trace)
        ...
        np.savez(os.path.join(args.output_path, "data.npz"), X=key_array, y=y)
