...
if args.num_tabs == 1:
    if args.loss == "HPCL" and len(train_y.shape) == 2:
        print(f"Detected hierarchical labels for HPCL: {train_y.shape}")
        # For HPCL, pass both levels of num_classes [num_websites, num_webpages]
        num_classes = [len(np.unique(train_y[:, 0])), len(np.unique(train_y[:, 1]))]
    else:
...
if args.model in ["BAPM", "TMWF"]: # Assume num_tabs is known
    ...
    # HF model requires hierarchical class structure
...
if args.load_file is None:
    ...
    print("Loading the pretrained model in ", os.path.join(ckp_path, args.load_file))
    checkpoint = torch.load(os.path.join(ckp_path, args.load_file))
