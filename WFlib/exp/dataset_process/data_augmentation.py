...
def gen_augment(data, num_aug, effective_ranges, out_file):
    ...
    is_hierarchical = len(y.shape) == 2
    if is_hierarchical:
        print(f"Detected hierarchical labels with shape {y.shape}")
        print("Using webpage labels (second level) for data augmentation")
        # Extract webpage labels for augmentation logic, but keep original labels for output
        y_for_augmentation = y[:, 1]  # webpage labels
        y_original = y  # keep original hierarchical labels
    else:
        print("Using standard labels for data augmentation")
        y_for_augmentation = y
        y_original = y
    ...
    for index in tqdm(range(abs_X.shape[0])):
        ...
        cur_web = y_for_augmentation[index]  # Use webpage label for augmentation logic
        original_label = y_original[index]   # Use original label for output
...
temporal_data = np.load(os.path.join(ckp_path, f"attr_{args.attr_method}.npz"))["attr_values"]
