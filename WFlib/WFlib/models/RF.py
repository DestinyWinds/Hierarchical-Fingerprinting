...
class RF(nn.Module):
    def __init__(self, num_classes=100):
        ...
        features = make_layers([128, 128, 'M', 256, 256, 'M', 512])
        init_weights = True
        ...
        self.final_conv = nn.Sequential(
            nn.Conv1d(512, num_classes, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm1d(num_classes, eps=1e-05, momentum=0.1, affine=True),
            nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        if self.is_collecting:
            collected_features = self.classifier(x)
            collected_features = collected_features.view(collected_features.size(0), -1)
            self.collected_features.append(collected_features.detach().cpu())
