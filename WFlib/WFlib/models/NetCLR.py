...
class DFsimCLR(nn.Module):
    def __init__(self, df, out_dim):
        ...
        # Store original fc layer for feature extraction
        self.original_fc = self.backbone.fc
        
        # Replace with projection head
    ...
    def get_features(self, inp):
        """Extract features before projection head for kNN evaluation"""
        # Forward through backbone except the fc layer
        x = inp
        # Forward through NetCLR layers manually
        # ==== first block ====
        x = F.pad(x, (3,4))
        x = F.elu((self.backbone.conv1(x)))
        x = F.pad(x, (3,4))
        x = F.elu(self.backbone.batch_norm1(self.backbone.conv1_1(x)))
        x = F.pad(x, (3, 4))
        x = self.backbone.max_pool_1(x)
        ...
        x = F.pad(x, (3,4))
        x = F.relu(self.backbone.batch_norm2(self.backbone.conv2_2(x)))
        x = F.pad(x, (3,4))
        x = self.backbone.max_pool_2(x)
        x = self.backbone.dropout2(x)
        
        # ==== third block ====
        x = F.pad(x, (3,4))
        x = F.relu((self.backbone.conv3(x)))
        x = F.pad(x, (3,4))
        x = F.relu(self.backbone.batch_norm3(self.backbone.conv3_3(x)))
        x = F.pad(x, (3,4))
        x = self.backbone.max_pool_3(x)
        x = self.backbone.dropout3(x)
        
        # ==== fourth block ====
        x = F.pad(x, (3,4))
        x = F.relu((self.backbone.conv4(x)))
        x = F.pad(x, (3,4))
        x = F.relu(self.backbone.batch_norm4(self.backbone.conv4_4(x)))
        x = F.pad(x, (3,4))
        x = self.backbone.max_pool_4(x)
        x = self.backbone.dropout4(x)
        
        # Flatten to feature vector (before fc layer)
        x = x.view(x.size(0), -1)  # Shape: [batch_size, 5120]
        return x
